#!/bin/bash
#SBATCH --job-name=ld_adapt
#SBATCH --output=logs/liars_dice_opponent_adapt_%j.out
#SBATCH --error=logs/liars_dice_opponent_adapt_%j.err
#SBATCH --time=4:00:00
#SBATCH --mem=32G
...
echo "=== Opponent Adaptation: Trajectory Encoder (seed=$SEED) ==="
...
echo ""
echo "=== Opponent Adaptation: Grover Encoder (seed=$SEED) ==="
python eval_opponent_adaptation.py \
    --game "$GAME" \
    --encoder grover \
