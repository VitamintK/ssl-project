#!/bin/bash
#SBATCH --job-name=ld_adapt_ms
#SBATCH --output=logs/liars_dice_opponent_adapt_moreseeds_%j.out
#SBATCH --error=logs/liars_dice_opponent_adapt_moreseeds_%j.err
#SBATCH --time=4:00:00
#SBATCH --mem=32G
...
PAYOFFS="payoff_matrices/${SAFE_GAME}__seed${POOL_SEED}_payoffs.npz"

for SPLIT_SEED in 45 46 47 48 49 50 51; do
    echo ""
    echo "=== Opponent Adaptation: Trajectory Encoder (pool_seed=$POOL_SEED, split_seed=$SPLIT_SEED) ==="
    ...
    echo ""
    echo "=== Opponent Adaptation: Grover Encoder (pool_seed=$POOL_SEED, split_seed=$SPLIT_SEED) ==="
    python eval_opponent_adaptation.py \
        --game "$GAME" \
        --encoder grover \
